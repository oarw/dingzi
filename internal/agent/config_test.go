...
func TestCredentialPersistsAndURLIsIdempotent(t *testing.T) {
	...
	if _,err:=cfg.EnsureUUID();err!=nil { t.Fatal(err) }
	if err:=cfg.EnsureToken();err!=nil { t.Fatal(err) }
	loaded,err:=LoadConfig(path,Config{});if err!=nil { t.Fatal(err) }
	if err:=loaded.Validate();err!=nil { t.Fatal(err) }
	if loaded.Server!=cfg.Server || loaded.Token!=cfg.Token || loaded.UUID!=cfg.UUID { t.Fatal("restart changed identity or appended endpoint") }
	if err:=loaded.EnsureToken();err!=nil { t.Fatal(err) }
	if loaded.Token!=cfg.Token { t.Fatal("credential rotated on restart") }
}
